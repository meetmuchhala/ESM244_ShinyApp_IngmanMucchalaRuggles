---
title: "Initial shiny app infrastructure"
author: "Gabe Ingman, Meet Muchhala, Logan Ruggles"
format: 
  html:
    toc: true
editor: visual
...
(2) Open a new R script (not Quarto, just script)
...
(6) Here, we'll add a title panel, and a sidebar/main panel layout in our UI.
...
(7) Save, run app again (you may need to stop "listening" - press stop in Console)

## Add your first widget in the side panel!

See the Shiny widget gallery:<https://shiny.rstudio.com/gallery/widget-gallery.html>

(8) Within the sidebar panel, add a radioButtons() widget. You need to tell it:
...
(9) In the `server()`, create a reactive subset of the `penguins` df that only includes the species selected by the widget `species`. Then, create a reactive graph of the flipper length (`flipper_length_mm`) vs. body mass (`body_mass_g`) variables in ggplot, and have it show up in our UI (main panel)
...
2.  Note `penguin_select()` is followed by parentheses, indicating it is a *reactive* function, not just a regular dataframe!

<!-- -->

(10) Try running the app again... notice that nothing shows up! That's because we haven't added our plot output to our UI yet! We need to call our graph (`output$penguin_plot`) back in the UI main panel:
...
(11) In the sidebar, create a new widget that is a dropdown selection (`selectInput`) for different colors that a user can choose from for the graph points:
...
(12) Run the app - and notice that a second widget shows up, but the graph doesn't change. Why not? Have we changed anything in the server that would let it know the color should be reactive to the widget input? Not yet! Let's do that next... back into the `server()`!
...
(13) First, create a reactive subset of the data (in the server) that you'll use to produce the table, and render the table in the server. The order of these in the server() isn't important, but keeping things organized in a logical manner IS VERY HELPFUL!
...
(14) Then call that `penguin_table` output so it shows up in the UI:
